...
for data_type in ['model','test']:
    for node_name in full_data[data_type]:
        ...
        for weight in full_data[data_type][node_name]:
            ...
            #print(weight)
            #print(current_conductance_time_avg)
            #averaged_data = np.average(current_conductance_time_avg)
            scaled_data[data_type][node_name]['weight'].append(weight)
            scaled_data[data_type][node_name]['avg_sense_vals'].append(current_conductance_time_avg)
...
for node_name in scaled_data['model']:
    ...
    for row in range(4):
        for col in range(4):
            current_taxel_sense_vals = avg_sense_vals[:,row,col]
            node_taxel_label = node_name+"_"+str(row)+"_"+str(col)
            # linear data scatter plots
            ax0.plot(weight, current_taxel_sense_vals, '.', label = node_taxel_label)
            ax2.plot(weight, current_taxel_sense_vals, '.', label = node_taxel_label)
                    
            # linear regression
            slope, intercept, r_value, p_value, std_err = linregress(weight, current_taxel_sense_vals)
            full_regression_param_node = (row,col,slope,intercept)
            # residuals
            ax1.plot(weight, weight * slope + intercept - current_taxel_sense_vals, '.', label = node_taxel_label)

            # Adds to global quantities
            full_regression_param[node_taxel_label] = full_regression_param_node
...
for taxel_name in full_regression_param:
    current_slope = full_regression_param[taxel_name][2] 
    current_offset = full_regression_param[taxel_name][3]
    node_name = taxel_name.split("_")[0]
    weight = scaled_data['model'][node_name]['weight']
    ax3.plot(weight, weight * current_slope + current_offset,'.', label = taxel_name + " regression")
...
average_slope = slope_sum/len(full_regression_param)
average_offset = offset_sum/len(full_regression_param)
...
for node_name in full_regression_param:
    slope = average_slope/full_regression_param[node_name][2]
    intercept = average_offset - full_regression_param[node_name][3]*average_slope/full_regression_param[node_name][2]
